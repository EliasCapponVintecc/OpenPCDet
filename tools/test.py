...
def eval_single_ckpt_modified(model, test_loader, args, eval_output_dir, logger, epoch_id, current_cfg, ckpt_path, dist_test=False):
    ...
    eval_utils.eval_one_epoch(
        current_cfg, args, model, test_loader, epoch_id, logger, dist_test=dist_test,
...
def parse_eval_config():
    ...
    parser.add_argument('--eval_config_file', type=str, required=True, help='YAML file listing models to evaluate')
    ...
    parser.add_argument('--save_to_file', action=argparse.BooleanOptionalAction, default=None, help='Save predictions. Overrides YAML global.')
    parser.add_argument('--infer_time', action=argparse.BooleanOptionalAction, default=None, help='Calculate inference latency. Overrides YAML global.')
    parser.add_argument('--launcher', choices=['none', 'pytorch', 'slurm'], default='none')
    parser.add_argument('--tcp_port', type=int, default=18888, help='tcp port for distrbuted training')
    parser.add_argument('--local_rank', type=int, default=None, help='local rank for distributed training')
    args = parser.parse_args()
    try:
        with open(args.eval_config_file, 'r') as f:
    ...
    for key in ['batch_size', 'workers', 'extra_tag', 'eval_tag', 'save_to_file', 'infer_time']:
        ...
        if final_val is not None:
             ...
             if key == 'batch_size': args.batch_size = 4
             if key == 'workers': args.workers = 4
             if key == 'extra_tag': args.extra_tag = 'multi_eval_default'
             if key == 'eval_tag': args.eval_tag = 'results'
             if key == 'save_to_file': args.save_to_file = False
             if key == 'infer_time': args.infer_time = False
    np.random.seed(1024)
...
def main():
    ...
    if args.batch_size % total_gpus != 0:
         print(f"Warning: Batch size {args.batch_size} not divisible by total GPUs {total_gpus}. Results might be inconsistent if batch size matters.")
    ...
    for model_info in models_to_evaluate:
        ...
        try:
            ...
            if tools_dir.exists():
                 ...
                 if local_rank == 0:
                     print(f"Warning: Cannot find 'tools' directory under {current_cfg.ROOT_DIR}. Cannot determine EXP_GROUP_PATH accurately.")
                 current_cfg.EXP_GROUP_PATH = Path(cfg_file).parent.name
        except ValueError as e:
             if local_rank == 0:
                 print(f"Warning: Could not automatically determine EXP_GROUP_PATH from {cfg_file} relative to {tools_dir}. Using cfg file parent dir name. Error: {e}")
             current_cfg.EXP_GROUP_PATH = Path(cfg_file).parent.name
        ...
        log_file = eval_output_dir / ('log_eval_%s_%s.txt' % (model_name, datetime.datetime.now().strftime('%Y%m%d-%H%M%S')))
        ...
        test_set, test_loader, sampler = build_dataloader(
            ...
            dist=dist_test, workers=current_workers, logger=logger, training=False,
